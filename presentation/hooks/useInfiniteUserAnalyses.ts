...
interface UseInfiniteUserAnalysesInput {
...
export function useInfiniteUserAnalyses({
  initialPage,
  limit,
}: UseInfiniteUserAnalysesInput) {
  ...
  async function loadMore() {
    ...
    try {
      ...
      const res = await fetch(`/api/dashboard/analyses?${params}`, {
        cache: "no-store",
      });
      if (!res.ok) {
      ...
      const page = (await res.json()) as UserAnalysesPageDto;
  ...
  return {
    items,
    totalCount: initialPage.counts.total,
    hasMore,
    loading,
    error,
    sentinelRef,
